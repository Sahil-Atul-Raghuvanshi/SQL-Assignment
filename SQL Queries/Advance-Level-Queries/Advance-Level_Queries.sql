...
-- 12> Partition the sales data to compare the performance of different regions and identify any anomalies.
	...
	Anomalies as (
		select c.region,o.order_id,o.total_amount,sm.avg ,sm.std,
		(case 
			when o.total_amount > (sm.avg+2 * sm.std) then 'High Anomaly'
			when o.total_amount < (sm.avg-2 * sm.std) then 'Low Anomaly'
			else 'Normal'

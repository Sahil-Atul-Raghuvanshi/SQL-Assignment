...
-- Approach 1
	...
	cte2 as (
		select pn,pc,no_of_times_ordered,
		dense_rank() over(partition by pc order by no_of_times_ordered desc) as rnk
		from cte1 
	)
	select pn as product_name,pc as category,no_of_times_ordered from cte2 where rnk=1;
	----------------------------------------------------------------------------------------
	-- Approach 3
		with cte1 as (
		select p.product_name as pn,p.category as pc,count(p.product_name) as no_of_times_ordered
	...
	cte2 as (
		select pn,pc,no_of_times_ordered,
		row_number() over(partition by pc order by no_of_times_ordered desc) as rn
		from cte1 
	)
	select pn as product_name,pc as category,no_of_times_ordered from cte2 where rn=1;
